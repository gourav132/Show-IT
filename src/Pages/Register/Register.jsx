...
function Register() {
  ...
  const register = () => {
    if (!authentication.name) alert("Please enter name");
    registerWithEmailAndPassword(authentication.name, authentication.email, authentication.password);
  };

  useEffect(() => {
    if (loading) return;
    if (user) history("/Create");
  }, [user, loading]);
  ...
  return (
    <div>
      ...
      <div className="login">
        ...
        {inputs("Full Name", "name")}
        {inputs("Email address", "email")}
        {inputs("Password", "password")}
          <button className="mt-6 login__btn font-medium text-white mb-4 py-2 rounded-md" onClick={register}>
            Register
          </button>
          <div class="">
