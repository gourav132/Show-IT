...
export default function Overview({ step, setStep }) {
  ...
  const list = (value, label, last = false) => {
    return (
      <li
        className={`w-full border-gray-200 sm:border-b-0 text-white ${
          !last ? "sm:border-r" : ""
      ...
      >
        <div className="flex items-center ps-3">
          <input
            ...
            checked={previewData.services.some(
    ...
    >
      ...
      <div>
        ...
        <textarea
          ...
          onChange={(e) => {
            setPreviewData({ ...previewData, overview: e.target.value });
          }}
        />
      </div>

      <h3 className="mt-4 mb-0.5 text-sm font-medium text-gray-200">
        Select your forte
      </h3>

      <ul className="items-center w-full text-sm font-medium text-white p-1 border border-gray-200 rounded-lg sm:flex dark:border-gray-600">
      ...
      <p className="mt-1 text-xs text-gray-400">
        you can select maximum four of your forte
      </p>

      <div className="w-full flex justify-end gap-4">
        <button
          className="px-6 py-2 bg-gray-500 rounded font-semibold"
          onClick={() => setStep("intro")}
        >
          Back
        </button>
        <button
          className="px-6 py-2 bg-purple-500 rounded font-semibold"
          onClick={() => setStep("exp")}
        >
          Next
        </button>
